...
@Component
@ConditionalOnProperty(name = "bots.angeimebel.enabled", havingValue = "true")
public class AngeimebelBot extends TelegramLongPollingBot {
    ...
    @Autowired
    public AngeimebelBot(CustomerRequestService customerRequestService,
        ...
        switch (step) {
            case ASK_EMAIL -> {
                if (!text.equalsIgnoreCase(BUTTON_SKIP)) {
                    if (!isValid(ASK_EMAIL, text)) {
                        sendMessage(chatId, "*❗ Email не коректний. Введіть ваш email ще раз* _(email@gmail.com)_:", true);
                        return;
                    }
                ...
                sendMessage(chatId, "*\uD83D\uDCF1 Введіть ваш мобільний номер телефону:*", true);
            }
            case ASK_PHONE -> {
                if (!isValid(ASK_PHONE, text)) {
                    sendMessage(chatId, "*❌ Мобільний не коректний. Введіть ваш мобільний номер ще раз* _(066 123 45 67)_:", true);
                    return;
                }
            ...
            case ASK_MESSAGE -> {
                if (!isValid(ASK_MESSAGE, text)) {
                    sendMessage(chatId, "⚠️ Звернення не може бути пустим. Напишіть текст вашого звернення:", true);
                    return;
                }
    ...
    private void sendAbout(Long chatId) {
        ...
        message.setText("👋 Привіт! Ми *Angeimebel* — стильні та якісні меблі на замовлення.");
        message.setParseMode("Markdown");

        InlineKeyboardButton socialButton = new InlineKeyboardButton();
        socialButton.setText("Наші соц. мережі");
        socialButton.setUrl("https://linktr.ee/angeimebel");

        List<InlineKeyboardButton> row = new ArrayList<>();
        row.add(socialButton);
